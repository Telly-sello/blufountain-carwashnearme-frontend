...
const Hero = () => {
  ...
  const Pickup = () => {
    return (
      <>
        <h2 className="flex justify-start text-base font-bold m-4">
          Carwash Information
        </h2>
        <form className="w-full border-2 border-gray-100  mb-4 ">
          <div className=" flex justify-between  my-2 ">
            <div className="flex flex-col w-1/3">
              <label
                className="flex ml-3 mt-1 font-bold justify-start"
                htmlForfor="date"
              >
                Date:
              </label>
            ...
            <div className="flex flex-col w-2/3 ">
              <label
                className="flex  mt-1 font-bold justify-start ml-10"
                htmlfor="returndate"
              >
  ...
  const Confirmation = () => {
    return (
      <>
        <h2 className="flex justify-start m-4 text-base font-bold">
        ...
        <div className="w-1/2 border-2 m-6 border-gray-100  flex flex-col justify-center">
          <div className="m-4 flex flex-col justify-start text-base ">
            <h2>
              Name: {stepperData.firstName} {stepperData.lastName}
            </h2>
  ...
  let price;
  function Prices() {
    if (heroData.bodytype === "Sedan" && heroData.service === "wash & go") {
      price = 70;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "wash & go"
    ) {
      price = 80;
    }
    if (
      heroData.bodytype === "Van/Minibus" &&
      heroData.service === "wash & go"
    ) {
      price = 100;
    }

    if (heroData.bodytype === "Sedan" && heroData.service === "wash dry") {
      price = 90;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "wash dry"
    ) {
      price = 100;
    }
    if (
    ...
    if (
      heroData.bodytype === "Sedan" &&
      heroData.service === "wash, dry & tyres"
    ) {
      price = 100;
    }
    if (
    ...
    if (heroData.bodytype === "Sedan" && heroData.service === "basic wash") {
      price = 110;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "basic wash"
    ) {
      price = 130;
    }
    if (
    ...
    if (heroData.bodytype === "Sedan" && heroData.service === "super wash") {
      price = 120;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "super wash"
    ) {
      price = 140;
    }
    if (
    ...
    if (heroData.bodytype === "Sedan" && heroData.service === "wash & wax") {
      price = 130;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "wash & wax"
    ) {
      price = 150;
    }
    if (
    ...
    if (heroData.bodytype === "Sedan" && heroData.service === "Full Valet") {
      price = 200;
    }
    if (
      heroData.bodytype === "SUV/Double cab" &&
      heroData.service === "Full Valet"
    ) {
      price = 250;
    }
    if (
  ...
  const sendEmail = async () => {
    try {
      await fetch(
        `http://localhost:8081/sendEmail/confirmProfileEmail?recipient=${stepperData.email}&subject=Test`,
        {
          mode: "no-cors",
          method: "POST",
        }
  ...
  const handleSubmit1 = () => {
    ...
    axios
      .post("http://localhost:3300/api/PostBooking", reqObject)
      .then((promise) => {
        sessionSuccess();
  ...
  const filteredBookings1 = filteredBookings?.filter((booking) => {
    if (selectedOption === "reference") {
      ...
      return booking?._id?.toLowerCase()?.includes(inputValue?.toLowerCase());
    }
      ...
      children: (
        <div>
          <div className="flex justify-end">
            <button
              className=" px-3 py-3 text-sm font-normal hover:text-blue-400 hover:bg-white-300 border-2 border-blue-300 rounded-lg bg-blue-400 text-white-500"
              onClick={() => showDrawer2()}
            >
              MY BOOKINGS
            </button>
          </div>
          <div className="mt-0 px- xl:px- mx-4 w-full " id="home">
            <div className="flex w-full justify-between">
                ...
                <div className="w-full mt-4 mb-4  border-b-2 border-gray-100">
                  ...
                  <div className="flex justify-between w-full ml-2 mb-6">
                    ...
                    >
                      ...
                      <option value="wash, dry & tyres">
                        Wash Dry & Tryes
                      </option>
                      <option value="basic wash">
                        Basic Wash (dry, vacuum, windows)
                      </option>
                      <option value="super wash">
                        Super Wash (basic & tyre shine)
                      </option>
                      <option value="wash & wax">
                        Wash & Wax (super wash & wax)
                      </option>
              ...
              <div className="flex flex-col w-3/4 ml-8 mt-4  border-solid  border-1 border-gray-100 rounded-lg bg-white-500 p-3">
                {filteredData.map((item) => {
                  return (
                    <div>
                      <div
                        className="flex justify-between w-full p-2 border-b-2 border-gray-100"
                        key={item.id}
                      >
                        <h2 className="font-medium text-base">
                          Blufontain {item.name} carwash is located at{" "}
                          {item.location}
                        </h2>
                      </div>

                      <h3 className="ml-3 mt-2 text-sm">
                      ...
                      <div className="flex flex-col full border-solid border-2 border-gray-100 rounded-lg m-2 p-2 mb-4 bg-white-300 shadow-md hover:shadow-xl">
                        ...
                        <button
                          className=" w-1/4 px-3 py-3 text-sm font-normal hover:text-blue-400 hover:bg-white-300 border-2 border-blue-300 rounded-lg bg-blue-400 text-white-500"
                          onClick={() => showStepper(item)}
                        >
                          Book Now
                        </button>
                      </div>
                    ...
                    >
                      <div>
                        <Tooltip placement="top" title="close" color="blue">
                          <button
                            className="absolute top-4 right-14 cursor-pointer p-1 border-2"
                            onClick={onClose2}
                          >
                      ...
                      <h4 className="text-xl text-blue-500 mb-2 ">
                        Car wash Bookings
                      </h4>
                          ...
                          <tbody>
                            {filteredBookings1.map((dat, index) => {
                              return (
                                <tr
                                  key={index}
                                  class="bg-white border-b border-gray-100  dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-500 "
                                >
                                  <th
                                    scope="row"
                  ...
                  <>
                    ...
                    >
                      <div>
                        <Tooltip placement="top" title="close" color="blue">
                          <button
                            className="absolute top-4 right-6 cursor-pointer p-1 border"
                            onClick={onClose1}
                          >
                      ...
                      <h4 className="text-xl text-blue-500 my-4">
                        Cancel Appointment
                      </h4>
                      <>
                        <div>
                          <h1 className="text-base mb-4">
                            Cancel booking for: {cancelBooking?.firstName}{" "}
                            {cancelBooking?.lastName}{" "}
                          </h1>
                  ...
                  <>
                    <div className="fixed insert-0 bg-black-500 bg-opacity-25 backdrop-blur-sm flex justify-center items-center w-full h-full justify-self-center top-0 right-0  z-50 ">
                      <div className="w-full flex flex-col h-4/5 justify-self-center fixed top-1/3 left-1/4 z-50 ">
                        ...
                        <Tooltip placement="top" title="close" color="blue">
                          <button
                            className="fixed top-1/4 right-1/4  cursor-pointer p-1 border-2 hover:border-blue-500"
  ...
  return (
    <>
      <div className="flex px-20 justify-between mt-40 w-full border border-gray-500">
